...
function normalizeEvent(raw) {
  return {
    ...
    nextScheduledDate: raw.recurrenceType && raw.recurrenceType !== "one-time"
...
function Calendar() {
  ...
  const {startDate, endDate, headerTitle} = useMemo(() => {
    if (viewMode === "month") {
      ...
      return {startDate: start, endDate: end, headerTitle: `${MONTH_NAMES[month]} ${year}`};
    }
    if (viewMode === "week") {
    ...
    return {
      startDate: fmt,
      endDate: fmt,
      headerTitle: d.toLocaleDateString("en-US", {weekday: "long", month: "long", day: "numeric", year: "numeric"}),
    };
  ...
  const handleViewModeChange = (mode) => {
    setViewMode(mode);
    if (mode !== "month") {
      setFocusedDate(new Date(year, month, today.getDate()));
    }
  };
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden">
      ...
      <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <main className="grow">
          <div className={`${PAGE_LAYOUT.list} px-6 sm:px-8 lg:px-10`}>
                  ...
                  <div className="flex flex-1 min-h-0">
                    <div className="w-16 shrink-0 border-r border-gray-200 dark:border-gray-700/60 flex flex-col">
                      {dayHours.map((h) => (
                        <div
                          key={h}
                          className="h-14 shrink-0 relative"
                        >
                          {h > 0 && (
                    ...
                    <div className="flex-1 grid grid-cols-7 overflow-auto min-w-0">
                      {dayHours.map((h) =>
                        weekDays.map((d) => {
                          ...
                          const dayEvents = getEventsForDate(d).filter((evt) => {
                              ...
                              >
                                ...
                                {event.scheduledTime && (
                                  <span className="opacity-90 text-[10px]">
                                    {(() => {
                                      const [hr, m] = event.scheduledTime.split(":");
